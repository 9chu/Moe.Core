...
# -*- coding: utf-8 -*-
# 生成UNICODE DATA
# REF: https://github.com/python/cpython/blob/master/Tools/unicode/makeunicodedata.py
...
def maketables(trace=0):
    ...
    with open("../src/UnicodeData.inl", "w", encoding="utf-8") as fp:
        #makeunicodename(unicode, trace)
        makeunicodedata(unicode, fp, trace)
        makeunicodetype(unicode, fp, trace)

# --------------------------------------------------------------------
# unicode character properties

def makeunicodedata(unicode, fp, trace):
    ...
    print("--- Writing...")
    print('static const unsigned kUnicodeCodePointCount = 0x%X;' % len(unicode.chars), file=fp)
    print('static const char* kUnicodeDataVersion = "%s";' % UNIDATA_VERSION, file=fp)
    print(file=fp)
    print("//////////////////////////////////////////////////////////////////////////////// UnicodeData", file=fp)
    print(file=fp)
    print("struct UnicodeDatabaseRecord", file=fp)
    print("{", file=fp)
    print("    const uint8_t Category;  // 到kUnicodeCategoryNames的索引", file=fp)
    print("    const uint8_t Combining;  // Combining分类值", file=fp)
    print("    const uint8_t Bidirectional;  // 到kUnicodeBidirectionalNames的索引", file=fp)
    print("    const uint8_t Mirrored;  // 如果Bidi中包含mirrored属性则为true", file=fp)
    print("    const uint8_t EastAsianWidth;  // 到kUnicodeEastAsianWidthNames的索引", file=fp)
    print("    const uint8_t NormalizationQuickCheck;", file=fp)
    print("};", file=fp)

    print(file=fp)
    print("// Unicode字符属性元组，元组都是唯一的，相同属性的Unicode会被映射到同一个元组上", file=fp)
    print("static const UnicodeDatabaseRecord kUnicodeDatabaseRecords[] = {", file=fp)
    for item in table:
    ...
    print(file=fp)
    print("// Unicode数据库索引表", file=fp)
    print("static const unsigned kUnicodeDatabaseRecordsIndexShift = %d;" % shift, file=fp)
    Array("kUnicodeDatabaseRecordsIndex1", index1).dump(fp, trace)
    Array("kUnicodeDatabaseRecordsIndex2", index2).dump(fp, trace)

    print("static const char* kUnicodeCategoryNames[] = {", file=fp)
    for name in CATEGORY_NAMES:
        print("    \"%s\"," % name, file=fp)
    print("    nullptr", file=fp)
    print("};", file=fp)

    print(file=fp)
    print("static const char* kUnicodeBidirectionalNames[] = {", file=fp)
    ...
    print(file=fp)
    print("static const char* kUnicodeEastAsianWidthNames[] = {", file=fp)
    ...
    print(file=fp)
    print("static const char* kUnicodeDecompPrefix[] = {", file=fp)
    ...
    print(file=fp)
    print("// NFC字符重新索引", file=fp)
    print("static const unsigned kUnicodeNfcFirstCount = %d;" % total_first, file=fp)
    print("static const unsigned kUnicodeNfcLastCount = %d;" % total_last, file=fp)

    print(file=fp)
    print("struct NfcCharReindex", file=fp)
    print("{", file=fp)
    print("    const unsigned Start;", file=fp)
    print("    const unsigned Count;", file=fp)
    print("    const unsigned Index;", file=fp)
    print("};", file=fp)

    print(file=fp)
    print("static const NfcCharReindex kUnicodeNfcFirst[] = {", file=fp)
    ...
    print(file=fp)
    print("static const NfcCharReindex kUnicodeNfcLast[] = {", file=fp)
    ...
    print(file=fp)
    print("// 解组合数据", file=fp)
    Array("kUnicodeDecompData", decomp_data).dump(fp, trace)

    print("// 解组合数据索引表", file=fp)
    print("static const unsigned kUnicodeDecompDataShift = %d;" % shift, file=fp)
    Array("kUnicodeDecompDataIndex1", index1).dump(fp, trace)
    Array("kUnicodeDecompDataIndex2", index2).dump(fp, trace)

    index, index2, shift = splitbins(comp_data, trace)
    print("// NFC对", file=fp)
    print("static const unsigned kUnicodeCompShift = %d;" % shift, file=fp)
    Array("kUnicodeCompDataIndex1", index).dump(fp, trace)
    Array("kUnicodeCompDataIndex2", index2).dump(fp, trace)
    ...
    for version, table, normalization in unicode.changed:
        ...
        index1, index2, shift = splitbins(index, trace)
        print("struct ChangeRecord", file=fp)
        print("{", file=fp)
        print("    const uint8_t BidirChanged;", file=fp)
        print("    const uint8_t CategoryChanged;", file=fp)
        print("    const uint8_t DecimalChanged;", file=fp)
        print("    const uint8_t MirroredChanged;", file=fp)
        print("    const uint8_t EastAsianWidthChanged;", file=fp)
        print("    const double NumericChanged;", file=fp)
        print("};", file=fp)
        print(file=fp)
        print("static const ChangeRecord kUnicodeChangeRecords_%s[] = {" % cversion, file=fp)
        ...
        print("    {", file=fp)
        print("        index = kUnicodeChangeRecordsIndex1_%s[n >> %d];" % (cversion, shift), file=fp)
        print("        index = kUnicodeChangeRecordsIndex2_%s[(index << %d) + (n & %d)];" % \
              (cversion, shift, ((1<<shift)-1)), file=fp)
        print("    }", file=fp)
        print("    return kUnicodeChangeRecords_%s[index];" % cversion, file=fp)
        print("}", file=fp)
...
def makeunicodetype(unicode, fp, trace):
    print("--- Preparing...")
    ...
    print("--- Writing...")
    print("//////////////////////////////////////////////////////////////////////////////// UnicodeTypes", file=fp)
    print(file=fp)
    print("struct UnicodeTypeRecord", file=fp)
    print("{", file=fp)
    print("    const int32_t Upper;  // 大写表示，存储到字符的偏移量或者是ExtendedCase表的索引", file=fp)
    print("    const int32_t Lower;  // 小写表示，同上", file=fp)
    print("    const int32_t Title;  // 首字母表示，同上", file=fp)
    print("    const uint8_t Decimal;", file=fp)
    print("    const uint8_t Digit;", file=fp)
    print("    const uint16_t Flags;", file=fp)
    print("};", file=fp)

    print(file=fp)
    print("// 类型描述元组，元组都是唯一的，相同类型记录的元组会被映射到同一个元组上", file=fp)
    print("static const UnicodeTypeRecord kUnicodeTypeRecords[] = {", file=fp)
    for item in table:
        print("    {%d, %d, %d, %d, %d, %d}," % item, file=fp)
    print("};", file=fp)

    print(file=fp)
    print("// 扩展大小写(ExtendedCase)映射表", file=fp)
    print("const char32_t kUnicodeExtendedCase[] = {", file=fp)
    for i in range(0, len(extra_casing)):
        c = extra_casing[i]
        if i % 8 == 0:
            if i != 0:
                print(file=fp)
    ...
    print(file=fp)
    print("// 类型索引表", file=fp)
    print("static const unsigned kUnicodeTypeRecordsIndexShift = %d;" % shift, file=fp)
    Array("kUnicodeTypeRecordsIndex1", index1).dump(fp, trace)
    Array("kUnicodeTypeRecordsIndex2", index2).dump(fp, trace)

    # 生成IsNumeric()函数
    numeric_items = sorted(numeric.items())
    print('bool moe::Unicode::IsNumeric(char32_t ch, double* out)noexcept', file=fp)
    print('{', file=fp)
    ...
    for value, codepoints in numeric_items:
        ...
        print('            if (out)', file=fp)
        print('                *out = %s;' % (value,), file=fp)
        print('            return true;', file=fp)
    print('        default:', file=fp)
    print('            if (out)', file=fp)
    print('                *out = 0.0;', file=fp)
    ...
    # 生成IsWhitespace()函数
    print(file=fp)
    print("// Whitespace = ch.bidi in ['WS', 'B', 'S'] or ch.category == 'Zs'", file=fp)
    print('bool moe::Unicode::IsWhitespace(char32_t ch)noexcept', file=fp)
    print('{', file=fp)
    print('    switch (ch)', file=fp)
    print('    {', file=fp)
    ...
    # 生成IsLinebreak()函数
    print(file=fp)
    print("// Linebreak = ch.props in ['BK', 'CR', 'LF', 'NL'] or ch.bidi == 'B'", file=fp)
    print('bool moe::Unicode::IsLinebreak(char32_t ch)noexcept', file=fp)
    print('{', file=fp)
    print('    switch (ch)', file=fp)
    print('    {', file=fp)
